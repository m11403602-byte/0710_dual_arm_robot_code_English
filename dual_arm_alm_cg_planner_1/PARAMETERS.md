...
對應 MATLAB 原型：`Dual_Arm_avoidance_system_v3.m`（外層）+ `Dual_Arm_Inequality_ALM_CG_v7.m`（內層）。
...
寫死於 `include/.../cg_solver.hpp` 成員預設值。**全部與 MATLAB CG v7 一致**。
...
| `K_inner_` | 200 | hpp:158 | `K_inner=200` | 內層 CG 最大迭代 |
