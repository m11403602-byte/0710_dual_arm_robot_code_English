...
SolverLog CgSolver::run_alm()
{
  ...
  for (int k_outer = 1; k_outer <= K_outer_; ++k_outer) {
    ...
    // ===== [Step 3] 乘子更新 mu = max(0, mu + c*g) + safeguard =====
    double mu_max_new = 0.0;
    for (int ii = 0; ii < num_C_; ++ii) {
      double val = mu_loc(ii) + c_loc * g_all(ii);
      if (val < 0.0) val = 0.0;
      mu_loc(ii) = val;
      if (val > mu_max_new) mu_max_new = val;
    }
    if (mu_max_new > mu_max_safeguard_) {
      ++log.mu_reset_count;
      std::cout << "  * [Safeguard #" << log.mu_reset_count
                << "] mu_max=" << mu_max_new << " 超限, reset 0\n";
      mu_loc.setZero();
    }
