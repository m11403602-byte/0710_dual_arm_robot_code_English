# dual_arm_alm_cg_planner_1

雙臂避障路徑規劃器 — ROS 2 Humble / MoveIt2 插件。

由 MATLAB 原型（`Dual_Arm_avoidance_system_v3` + `Dual_Arm_Inequality_ALM_CG_v7`）轉寫為 C++17，使用 Eigen3。內層採用 **ALM (PHR 增廣拉格朗日) + CG-FR (Fletcher-Reeves 共軛梯度) + 1D Newton 線搜索**，無 Hessian、無 LDLT。
...
內層 (cg_solver):         ALM 外層 (罰參數/乘子更新) + CG-FR 內層 + 線搜索,
...
所有原始碼（核心演算法 + MoveIt 插件介面）編進**單一共享庫** `libdual_arm_alm_cg_planner_1.so`。

| 組成 | 角色 |
|------|------|
| `cg_solver` | 第 1 層: 內層 CG 求解器 + FK + 包覆球 + 危險因子 |
...
├── CMakeLists.txt
├── package.xml
├── dual_arm_alm_cg_planner_1.xml        # plugin 描述
├── README.md
├── config/
│   └── dual_arm_alm_cg_planner_1.yaml   # 規劃器參數
├── include/dual_arm_alm_cg_planner_1/
│   ├── cg_solver.hpp                       # 第 1 層: 內層 CG 求解器
...
```bash
cd ~/ros2_ws
colcon build --packages-select dual_arm_alm_cg_planner_1
...
```bash
ros2 run dual_arm_alm_cg_planner_1 run_standalone myout
# 或直接執行: ./install/.../lib/dual_arm_alm_cg_planner_1/run_standalone myout
```
...
```cpp
#include "dual_arm_alm_cg_planner_1/avoidance_system.hpp"
using namespace dual_arm_alm_cg_planner_1;
...
> 註：核心類別 `AvoidanceSystem` / `CgSolver` 本身只依賴 Eigen（程式碼層面無 ROS/MoveIt 呼叫）。但因合併為單一 .so，連結時會帶到 MoveIt 依賴，故需在含 MoveIt 的環境編譯。
...
```yaml
planning_plugin: dual_arm_alm_cg_planner_1/DualArmAlmCgPlannerManager
```

並載入 `config/dual_arm_alm_cg_planner_1.yaml` 的參數。規劃請求需含 joint goal constraints。
...
4. **包覆球參數**（`cg_solver.cpp` 的 `BUBBLES_*` / `PEDESTAL_*` 常數）：RA610/RA605 的球座標與半徑，依實際機型確認。
...
- **線性求解**：用 Eigen `colPivHouseholderQr`（spline 三對角）；內層 CG 本身不解線性系統。
