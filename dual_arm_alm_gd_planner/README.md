# <p align="center">dual_arm_alm_gd_planner</p>
...
<p align="justify">
Implemented in C++17 + Eigen3. The inner loop employs <b> ALM (Augmented Lagrangian) + GD (steepest descent) + 1D Newton line search </b> .
</p>
...
Inner level (gd_solver):         ALM outer loop (penalty parameter / multiplier update) + GD inner loop + line search
```
...
<p align="justify">
All source code (the core algorithm + the MoveIt plugin interface) is compiled into a <b> single shared library </b> <code>libdual_arm_alm_gd_planner.so</code> .
</p>
...
<table width="100%">
  ...
  <tbody>
    <tr>
      ...
      <td align="justify">Layer 1: inner GD solver + FK + bounding spheres + danger factor</td>
    </tr>
    <tr>
...
### Inner Level: ALM Gradient-Descent Solver ( <code>GdSolver::run_alm</code> )
...
│ │ ┌─ inner GD iteration (up to alm_k_inner=200 steps) ───────────────┐  │ │
...
│ │      --No--> back to the inner GD iteration                           │ │
...
<ul>
  ...
  <li align="justify" style="margin-bottom: 8px;"><b> <code>include/dual_arm_alm_gd_planner/</code> </b> : <b> the interface-declaration layer </b> .</li>
...
├── CMakeLists.txt
├── package.xml
├── dual_arm_alm_gd_planner.xml
├── README.md
├── PARAMETERS.md
├── config/
│   └── dual_arm_alm_gd_planning.yaml
├── include/dual_arm_alm_gd_planner/
│   ├── gd_solver.hpp
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr>
      <td align="justify"><code>dual_arm_alm_gd_planner.xml</code></td>
    ...
    <tr>
      <td align="justify"><code>config/dual_arm_alm_gd_planning.yaml</code></td>
    ...
    <tr>
      ...
      <td align="justify">Layer 1: FK, bounding spheres, danger factor, ALM + GD inner solve</td>
    </tr>
    <tr>
...
```bash
cd ~/ros2_ws
colcon build --packages-select dual_arm_alm_gd_planner
...
<ol>
  <li align="justify" style="margin-bottom: 8px;">Copy this package's <code>config/dual_arm_alm_gd_planning.yaml</code> to <code>hiwin_dual_arm/config/</code> (the filename differs from planner to planner). The file already contains the complete <code>planning_plugin</code> and <code>request_adapters</code> settings, so copying it suffices — no further pipeline changes are required.</li>
