...
//   call the inner GD optimization -> Spline reconstruction -> re-check (up to max_refinement_iter rounds)
...
// indices returned by find_collision_targets
...
struct IterLogEntry {
  ...
  SolverLog           solver_log;     // this round's inner GD log
};

class AvoidanceSystem
{
public:
  // constructor: takes A_waypoints, B_waypoints, path_weight, danger_threshold (default 0.4)
  ...
  AvoidanceSystem(const Eigen::MatrixXd& A_waypoints,
                  ...
                  // the following are tunable parameters (defaults used if not passed)
  ...
  // [NEW] iteration-limit injection (yaml-tunable)
  void set_alm_iters(int k_outer, int k_inner) { alm_k_outer_ = k_outer; alm_k_inner_ = k_inner; }

  // [NEW] ALM parameter injection (yaml-tunable; if not called, this solver's recommended defaults are used)
  //   ⚠ c0 must not exceed c_max (multiplier update is classical PHR, no upper-bound safeguarding)
  ...
  static double gv_clamp(double g) { return (g > 0.0 && g < 1.0) ? g : 0.5; }  // gamma_v must lie in (0,1)
...
private:
  ...
  // [NEW] ALM parameters (default = GD recommended values)
  ...
  // call the inner GD: returns Xa_opt/Xb_opt (M x 6) + log
