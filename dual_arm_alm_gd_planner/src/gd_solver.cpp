// =====================================================================
// gd_solver.cpp — Layer 1 GD solver implementation
// =====================================================================
...
// =====================================================================
// bounding-sphere constants for each robot's arm/pedestal geometry
// =====================================================================
...
// rotation matrix about axis by angle_deg (degrees)
...
// translation matrix along axis by dist_mm (mm)
...
// =====================================================================
// the base transform is already included in T_base; chain: T1=base*Tz(117), T2=Tz(448.5)*Rz(J1), ...
                                      ...
                                      Eigen::Matrix4d& T_ee)
{
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    for (const BubbleDef& b : BUBBLES_A) {
      if (b.link_id == i) {   // link_id aligned 0-indexed to the current cumulative transform
        Eigen::Vector4d pt(b.cx, b.cy, b.cz, 1.0);
    ...
    smooth_w_T_(smooth_w_T), smooth_w_neighbor_(smooth_w_neighbor)
{
  ...
  // mask -> linear indices (column-major flattening of the 16x18 mask)
...
Eigen::VectorXd GdSolver::compute_Dm(const Eigen::VectorXd& X, int m) const
{
  ...
  // flatten sj column-major into a 288-vector, then take lin_idx_AB
...
// =====================================================================
// compute_D_cache: D_base + D_plus (for forward differences; GD does not need D_minus)
// =====================================================================
...
// =====================================================================
// a <- a - phi'/phi''; phi',phi'' via 3-point central differences; falls back to fallback on failure
...
// =====================================================================
// run_alm: ALM outer + GD inner + line search
// =====================================================================
SolverLog GdSolver::run_alm()
{
  ...
  for (int k_outer = 1; k_outer <= K_outer_; ++k_outer) {
    ...
    for (int inner = 1; inner <= K_inner_now; ++inner) {
      ...
      // (1d) GD direction: steepest descent d = -G
      //   [vs CG] GD has no direction memory; no FR momentum bookkeeping (pre_G_norm2/pre_d) is kept
      Eigen::VectorXd d = -G;

      // (1e) 1D Newton line search
