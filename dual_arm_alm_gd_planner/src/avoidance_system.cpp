...
// =====================================================================
//   ⚠ targets/indices are 0-indexed throughout; conversions between 0-indexed
//     and 1-indexed-style counts (e.g. "row count" vs "last index") are annotated where they occur
// =====================================================================
...
// =====================================================================
// clamped_cubic_spline(t, [v0, Y, v1], tq): given endpoint slopes -> clamped cubic
...
// =====================================================================
// constructor: build robot base transforms, sampling parameters, and the initial spline trajectory
// =====================================================================
    ...
    smooth_w_T_(smooth_w_T), smooth_w_neighbor_(smooth_w_neighbor)
{
  ...
  // base transforms: Arm A Ty(700)Rz(180), Arm B Ty(-700)Rz(0)
...
void AvoidanceSystem::generate_initial_trajectory()
{
  ...
  // dist = max chord length between waypoints of the two arms
  ...
  // clamped boundary condition: v_start=v_end=0; Y = Anchors transposed (12 x 2)
                                      ...
                                      Eigen::MatrixXd* path_D_all) const
{
  ...
  // first compute step 0 to obtain the D length (full constraints)
  //   run once through GdSolver's FK+calc_df+mask to obtain num_D
  auto compute_step_D = [&](int t) -> Eigen::VectorXd {
    ...
    // column-major extraction of the in-mask elements (consistent with gd_solver)
...
{
  const double threshold = danger_threshold_;   // uses 0.4 (not +tol)
  const int n = static_cast<int>(path_D.size());

  // all_danger_indices = indices where path_D >= threshold (0-indexed)
  ...
  // q1: search minidx..Max_idx-1 for D close to (threshold+Max_val)/2
  ...
  // boundary protection: keep at least one margin sample on each side
  ...
  Head = std::max(Head, 0);       // clamp to the first valid 0-indexed sample
  Tail = std::min(Tail, n - 1);   // clamp to the last valid 0-indexed sample
...
// =====================================================================
// run_solver_global: call the inner GD
// =====================================================================
                                        ...
                                        SolverLog& solver_log)
{
  ...
  // X = trajectory.pos rows selected by targets (5 x 12)
  ...
  solver.set_alm_params(alm_mu0_, alm_c0_, alm_c_max_,
                        alm_beta_c_, alm_gamma_v_);   // [NEW] yaml → ALM injection (multiplier update is pure PHR, no mu upper bound/reset)
  solver.set_iter_limits(alm_k_outer_, alm_k_inner_);   // [NEW] yaml → iteration-limit injection
  solver_log = solver.run_alm();

  // P = number of interior points = targets.size()-2; unpack V_final into per-point 12-vectors
                                                         ...
                                                         std::vector<int>& targets_out) const
{
  ...
  // if fewer than 2 valid anchors remain: degenerate (anchors nearly all coincident)
  ...
  if (static_cast<int>(valid_rows.size()) < 2) {
    ...
    // otherwise: Clean = Patch rows selected by valid_rows; t_knots = [0; cumsum(seg_dists)]
  ...
  // n_steps_all = ceil(seg/STEP_MAX); drop-tail concatenation
  ...
  // ===== Concatenation: head rows + reconstructed patch interior + tail rows =====
  // patch_core = Patch_Pos rows 1..end-1 (drop head and tail, use the actual row count)
  //   num_patch = size(patch_core, 1) = Patch_Pos.rows() - 2  (use the actual row count, not T_patch)
  const int patch_rows = static_cast<int>(Patch_Pos.rows());
  const int num_patch  = patch_rows - 2;
  // num_head = number of untouched rows before the patched region (0-indexed targets.front() -> count = index+1)
  const int num_head = targets.front() + 1;
  // num_tail = number of untouched rows after the patched region
  const int num_tail = total_len - targets.back();
  // T_new = total row count of the regenerated trajectory
  const int T_new = num_head + num_patch + num_tail;

  // whole-segment assignment via Eigen block operations
  //   the degenerate branch above (fewer than 2 valid anchors -> 2-point spline) still yields num_patch>=0 here
  Eigen::MatrixXd final_pos(T_new, 12);
  // final_pos[0 .. num_head-1] = pos_all[0 .. num_head-1] (untouched head rows)
  final_pos.topRows(num_head) = pos_all.topRows(num_head);
  // final_pos[num_head .. num_head+num_patch-1] = patch_core (Patch_Pos with head and tail dropped)
  if (num_patch > 0)
    final_pos.middleRows(num_head, num_patch) = Patch_Pos.middleRows(1, num_patch);
  // final_pos[num_head+num_patch .. end] = pos_all[targets.back() .. end] (untouched tail rows, num_tail rows total)
  ...
  //   only computed exactly when every anchor stayed valid and there are exactly 4 segments
  targets_out.clear();
  if (static_cast<int>(valid_rows.size()) == P + 2 && n_steps_all.size() == 4) {
    // cum_steps = cumsum(n_steps_all);
    //   targets_out = [num_head, num_head+cum(1), num_head+cum(2), num_head+cum(3), num_head+num_patch+1]
    //   all in 0-indexed terms (num_head above is a 1-indexed count, so subtract 1 first)
    ...
    // degenerate fallback: evenly spaced indices from num_head to num_head+num_patch+1
...
void AvoidanceSystem::run_optimization()
{
  ...
  // start/goal pre-check (danger_threshold_+collision_tolerance_ threshold + warning + early return)
...
void AvoidanceSystem::export_unified(const std::string& prefix, int level) const
{
  ...
  // ---- 2. summary.csv: per-round scoreboard (merging the old #2 eight columns + old #4's unique six columns = 14 columns) ----
  {
    Eigen::MatrixXd S(n_outer, 14);
    for (int k = 0; k < n_outer; ++k) {
      const SolverLog& lg = iter_log_[k].solver_log;
      const int idx = std::max(1, lg.outer_iter) - 1;   // last round (0-indexed)
      S(k,0)  = k + 1;
  ...
  // ---- 3. inner.csv: concatenated inner steps (= old diag_inner) ----
  {
    int total = 0;
