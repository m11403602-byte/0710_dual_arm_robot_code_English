# 參數對照表 — dual_arm_alm_newton_planner_1

本文件列出所有參數：外部可調 vs 寫死、預設值、所在位置、對應 MATLAB。

對應 MATLAB 原型：`Dual_Arm_avoidance_system_v3.m`（外層）+ `Dual_Arm_Inequality_ALM_Newton_v6.m`（內層）。
...
> 傳遞路徑：yaml → planner_manager（initialize 讀入）→ AvoidanceSystem 建構函數 → NewtonSolver 建構函數（smooth_w 系列）。standalone/test 用 4 參數呼叫仍相容（新參數有預設值）。
...
## 三、內層 ALM 參數（newton_solver）

寫死於 `include/.../newton_solver.hpp` 成員預設值。**全部與 MATLAB Newton v6 一致**。
...
| `K_inner_` | 200 | hpp:158 | `K_inner=200` | 內層 Newton 最大迭代 |
...
## 四、平滑項權重（newton_solver）

**已開放為 yaml 可調**（見第一節）。控制軌跡平滑度，預設與 MATLAB 一致。
傳遞：yaml → AvoidanceSystem → NewtonSolver 建構函數。
...
## 五、數值保護（newton_solver）

寫死於 `src/newton_solver.cpp` run_alm 內。

| 參數 | 值 | 位置 | 說明 |
|------|-----|------|------|
| 發散門檻 | ‖G‖ > 1e10 | cpp:528 | 梯度爆炸視為發散 |
| 方向發散 | d 含 NaN/Inf | run_alm (1e) | Hessian 病態 → LDLT 解出非有限值即中止 |

---

## 六、危險因子公式（newton_solver）

寫死於 `src/newton_solver.cpp` `calc_df()`。**核心避障數學**。
...
## 八、機器人幾何（newton_solver）

寫死於 `src/newton_solver.cpp`。換機器人需整批修改。
...
| 改收斂嚴格度 | `epsilon_v/g/compl` | ✗ newton_solver.hpp（重編）|
| 改 ALM 收斂行為 | `c_0/mu_0/beta_c` | ✗ newton_solver.hpp（重編，進階）|
| 換機器人幾何 | 包覆球 + FK + base | ✗ newton_solver.cpp + planner_manager.cpp（重編）|
...
| `alm_c_max` | 100 | 罰參數上限（Newton 整定: Hessian 條件數保護） |
| `alm_beta_c` | 2 | 罰參數放大係數（Newton 整定: 溫和爬升） |
