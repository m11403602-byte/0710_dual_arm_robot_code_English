...
由 MATLAB 原型（`Dual_Arm_avoidance_system_v3` + `Dual_Arm_Inequality_ALM_Newton_v6`）轉寫為 C++17，使用 Eigen3。內層採用 **ALM (PHR 增廣拉格朗日) + Newton 法 (d = -(H\\G), LDLT 分解, alpha=1 純 Newton 步)**。Hessian = H_smooth + H_collision（中心差分 + block-diagonal 雙擾動）。
...
├── CMakeLists.txt
├── package.xml
├── dual_arm_alm_newton_planner_1.xml        # plugin 描述
├── README.md
├── config/
│   └── dual_arm_alm_newton_planner_1.yaml   # 規劃器參數
├── include/dual_arm_alm_newton_planner_1/
│   ├── newton_solver.hpp                       # 第 1 層: 內層 Newton 求解器
...
```bash
ros2 run dual_arm_alm_newton_planner_1 run_standalone myout
# 或直接執行: ./install/.../lib/dual_arm_alm_newton_planner_1/run_standalone myout
```
...
> 註：核心類別 `AvoidanceSystem` / `NewtonSolver` 本身只依賴 Eigen（程式碼層面無 ROS/MoveIt 呼叫）。但因合併為單一 .so，連結時會帶到 MoveIt 依賴，故需在含 MoveIt 的環境編譯。
...
並載入 `config/dual_arm_alm_newton_planner_1.yaml` 的參數。規劃請求需含 joint goal constraints。
...
- **線性求解**：用 Eigen `colPivHouseholderQr`（spline 三對角）；內層 Newton 用 Eigen `LDLT` 解 d = -(H\\G)。
