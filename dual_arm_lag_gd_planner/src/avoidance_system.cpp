...
#include "dual_arm_lag_gd_planner/data_io.hpp"
...
// =====================================================================
// constructor: set robot base transforms, tunable parameters, initial trajectory
// =====================================================================
                                      ...
                                      Eigen::MatrixXd* path_D_all) const
{
  ...
  // first compute step 0 to obtain the D length, by running once through
  //   GdSolver's FK+calc_df+mask to obtain num_D (full constraints)
...
{
  const double threshold = danger_threshold_;   // uses the plain threshold (not +tol)
  const int n = static_cast<int>(path_D.size());

  // all_danger_indices = steps where path_D >= threshold (0-indexed)
  ...
  // q3: maxidx..Max_idx+1 scanned in reverse, same closest-value search
  ...
  // boundary protection: clamp minidx>=1, maxidx<=n-2 (0-indexed)
                                        ...
                                        SolverLog& solver_log)
{
  ...
  // X = trajectory.pos at the target indices (5 x 12)
  ...
  // P = number of interior points; unpack V_final's X segment into Xa_opt/Xb_opt (column-major)
                                                         ...
                                                         std::vector<int>& targets_out) const
{
  ...
  //   valid_rows tracks which rows are kept; its size = number of retained anchors
  ...
  if (static_cast<int>(valid_rows.size()) < 2) {
    ...
    seg_dists.assign(1, 1e-6);
    t_knots.resize(2);
    t_knots << 0.0, 1e-6;
  } else {
    // otherwise: Clean = Patch rows at valid_rows; t_knots = [0; cumsum(dist_seg)]
  ...
  // ===== Concatenation: old-head rows + reconstructed patch interior + old-tail rows =====
  // patch_core = Patch_Pos with head and tail rows dropped
  //   num_patch = actual row count of patch_core (Patch_Pos.rows() - 2), not the nominal T_patch
  ...
  // num_tail = number of untouched rows from targets.back() to the end of the old trajectory
  const int num_tail = total_len - targets.back();
  // T_new = total row count of the reconstructed trajectory
  ...
  // final_pos[0 .. num_head-1] = pos_all[0 .. num_head-1] (untouched head)
  ...
  // final_pos[num_head+num_patch ..] = pos_all[targets.back() ..] (untouched tail, num_tail rows)
  ...
  //   only computed exactly when all anchors stayed valid and there are exactly 4 segments
  targets_out.clear();
  if (static_cast<int>(valid_rows.size()) == P + 2 && n_steps_all.size() == 4) {
    // cumulative step counts locate each interior anchor within the patch
    int c1 = n_steps_all[0], c2 = c1 + n_steps_all[1], c3 = c2 + n_steps_all[2];
    const int nh = num_head - 1;   // 0-indexed base
    targets_out = {nh, nh + c1, nh + c2, nh + c3, nh + num_patch + 1};
  } else {
    // degenerate: approximate with evenly spaced indices across the patch
...
void AvoidanceSystem::run_optimization()
{
  ...
  // start/goal pre-check (threshold + warning + early return)
