    ...
    smooth_w_T_(smooth_w_T), smooth_w_neighbor_(smooth_w_neighbor)
{
  ...
  std::cout << "  [Init] Max_D=" << compute_Dx_all(Xm_initial_.head(num_X_)).maxCoeff()
            << " | Method=GD+1D-Newton (pure Lagrangian)\n";
...
// =====================================================================
Eigen::VectorXd GdSolver::compute_G(const Eigen::VectorXd& V,
...
// =====================================================================
double GdSolver::cost_function_F(const Eigen::VectorXd& V) const
{
  double f, fa, fb;
