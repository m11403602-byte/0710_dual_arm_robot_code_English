# <p align="center">dual_arm_lag_cg_planner</p>
...
<p align="justify">
Implemented in C++17 + Eigen3. The inner loop employs <b> a pure Lagrangian formulation (decision variable V=[X;λ;S]) + CG-FR (Fletcher-Reeves conjugate gradient) + 1D Newton line search </b> .
</p>
...
Inner level (cg_solver):         single-loop pure Lagrangian (V=[X;λ;S] ∈ ℝ^1116) + CG-FR + line search
```
...
<p align="justify">
All source code (the core algorithm + the MoveIt plugin interface) is compiled into a <b> single shared library </b> <code>libdual_arm_lag_cg_planner.so</code> .
</p>
...
<table width="100%">
  ...
  <tbody>
    <tr>
      ...
      <td align="justify">Layer 1: inner CG solver (V=[X;λ;S]) + FK + bounding spheres + danger factor</td>
    </tr>
    <tr>
...
### Inner Level: Pure Lagrangian Conjugate-Gradient Solver ( <code>CgSolver::run_lag</code> )
...
<ul>
  ...
  <li align="justify" style="margin-bottom: 8px;"><b> <code>config/</code> </b> : <b> the runtime-parameter layer </b> — stores the yaml parameters loaded when move_group starts; must be copied as <code>hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml</code> .</li>
  <li align="justify" style="margin-bottom: 8px;"><b> <code>include/dual_arm_lag_cg_planner/</code> </b> : <b> the interface-declaration layer </b> .</li>
  <li align="justify" style="margin-bottom: 8px;"><b> <code>src/</code> </b> : <b> the algorithm-implementation layer </b> .</li>
</ul>

```
dual_arm_lag_cg_planner/
├── CMakeLists.txt
├── package.xml
├── dual_arm_lag_cg_planner.xml
├── README.md
├── PARAMETERS.md
├── config/
│   └── dual_arm_lag_cg_planner.yaml
├── include/dual_arm_lag_cg_planner/
│   ├── cg_solver.hpp
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr>
      <td align="justify"><code>dual_arm_lag_cg_planner.xml</code></td>
    ...
    <tr>
      <td align="justify"><code>config/dual_arm_lag_cg_planner.yaml</code></td>
      <td align="justify">Planner parameters (must be copied as <code>hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml</code> )</td>
    </tr>
    <tr>
      <td align="justify"><code>cg_solver.hpp/.cpp</code></td>
      <td align="justify">Layer 1: FK, bounding spheres, danger factor, pure Lagrangian CG solve</td>
    </tr>
    <tr>
...
```bash
cd ~/ros2_ws
colcon build --packages-select dual_arm_lag_cg_planner
...
<ol>
  <li align="justify" style="margin-bottom: 8px;">Copy this package's <code>config/dual_arm_lag_cg_planner.yaml</code> as <code>hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml</code> (the filename differs from planner to planner). The file already contains the complete <code>planning_plugin</code> and <code>request_adapters</code> settings, so copying it suffices — no further pipeline changes are required.</li>
...
<ol>
  ...
  <li align="justify" style="margin-bottom: 8px;"><b> Bounding-sphere parameters </b> ( the <code>BUBBLES_*</code> / <code>PEDESTAL_*</code> constants in <code>cg_solver.cpp</code> ) : the sphere coordinates and radii of RA610/RA605 — confirm them against your actual robot model.</li>
</ol>
