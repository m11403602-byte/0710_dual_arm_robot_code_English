...
// =====================================================================
//   clamped_cubic_spline: given endpoint slopes v0/v1 -> clamped cubic through Y at t_knots, sampled at t_query
...
// =====================================================================
// constructor: set robot bases, tunable parameters, then build the initial trajectory
// =====================================================================
...
void AvoidanceSystem::generate_initial_trajectory()
{
  ...
  // clamped boundary slopes v_start=v_end=0; Y = Anchors' (12 x 2)
...
CollisionIndices AvoidanceSystem::find_collision_targets(const Eigen::VectorXd& path_D) const
{
  ...
  // danger indices: steps where path_D >= threshold (0-indexed)
  ...
  // boundary protection: keep minidx>=1, maxidx<=n-2
  ...
  Head = std::max(Head, 0);       // clamp to valid range
  Tail = std::min(Tail, n - 1);
                                        ...
                                        SolverLog& solver_log)
{
  ...
  // P = number of interior points; unpack V_final's X segment into Xa_opt/Xb_opt
                                                         ...
                                                         std::vector<int>& targets_out) const
{
  ...
  // segment distance filter: drop points closer than 1e-4 to the previous anchor (coincident points)
  ...
  // degenerate case (anchors nearly all coincident): keep only head and tail, tiny fixed spacing
  ...
  if (static_cast<int>(valid_rows.size()) < 2) {
    ...
    // normal case: keep only the retained anchors; t_knots = [0; cumsum(seg_dists)]
  ...
  // n_steps_all = ceil(seg/STEP_MAX) per segment; drop-tail concatenation
  ...
  // ===== Concatenation: unchanged head segment + reconstructed patch + unchanged tail segment =====
  // num_patch = interior rows of Patch_Pos, dropping its head and tail
  ...
  // targets_out: the 5 anchors' indices in the new trajectory (0-indexed)
  targets_out.clear();
  if (static_cast<int>(valid_rows.size()) == P + 2 && n_steps_all.size() == 4) {
    // cumulative step counts locate each of the 5 anchors in the reconstructed patch
    ...
    // degenerate: approximate the 5 anchors as evenly spaced
