...
// =====================================================================
// clamped_cubic_spline: given endpoint slopes -> clamped cubic
...
// =====================================================================
// constructor: sets robot bases, tunable parameters, and generates the initial trajectory
// =====================================================================
...
void AvoidanceSystem::generate_initial_trajectory()
{
  ...
  // clamped: v_start=v_end=0; Y = Anchors' (12 x 2)
                                      ...
                                      Eigen::MatrixXd* path_D_all) const
{
  ...
  // run once through NewtonSolver's FK+calc_df+mask to obtain the D length (num_D, full constraints)
  auto compute_step_D = [&](int t) -> Eigen::VectorXd {
    ...
    // column-major extraction of the in-mask elements (consistent with newton_solver)
...
CollisionIndices AvoidanceSystem::find_collision_targets(const Eigen::VectorXd& path_D) const
{
  ...
  // danger indices = all i where path_D(i) >= threshold (0-indexed)
  ...
  // boundary protection, 0-indexed: minidx>=1, maxidx<=n-2
...
// =====================================================================
// run_solver_global: call the inner Newton (pure Lagrangian)
// =====================================================================
                                        ...
                                        SolverLog& solver_log)
{
  ...
  solver.set_lag_params(lag_wd_, lag_lam0_, lag_s0_,
                        lag_tol_phys_, lag_tol_stable_, lag_tol_stat_, lag_max_iter_);
  solver.set_iter_limits(lag_max_iter_);   // [NEW] iteration-limit injection
  solver.set_verbose(solver_verbose_);   // [NEW] pass through the yaml switch
  solver_log = solver.run_lag();

  // P = number of interior points = size(X,1)-2; unpack V_final into per-point Xa/Xb rows
                                                         ...
                                                         std::vector<int>& targets_out) const
{
  ...
  // Patch = [Head point; optimized points (P); Tail point] ((P+2) x 12)
  ...
  // if fewer than 2 valid rows: degenerate (anchors nearly all coincident)
  //   keep only head and tail, with a nominal tiny segment distance
  Eigen::MatrixXd Clean;
  Eigen::VectorXd t_knots;
  if (static_cast<int>(valid_rows.size()) < 2) {
    Clean.resize(2, 12);
    Clean.row(0) = Patch.row(0);          // head
    Clean.row(1) = Patch.row(P + 1);      // tail
    seg_dists.assign(1, 1e-6);            // nominal tiny segment distance
    t_knots.resize(2);
    t_knots << 0.0, 1e-6;
  } else {
    // otherwise: Clean = the valid rows of Patch; t_knots = [0; cumsum(dist_seg)]
  ...
  // ===== Concatenation =====
  // patch_core = Patch_Pos rows [1, rows-1) -> drop head and tail, use the actual row count
  //   num_patch = Patch_Pos.rows() - 2  (use the actual row count, not T_patch)
  const int patch_rows = static_cast<int>(Patch_Pos.rows());
  const int num_patch  = patch_rows - 2;
  // num_head = number of untouched rows before targets.front() (inclusive, 0-indexed -> count = index+1)
  const int num_head = targets.front() + 1;
  // num_tail = number of untouched rows from targets.back() to the end
  const int num_tail = total_len - targets.back();
  // T_new = num_head + num_patch + num_tail
  const int T_new = num_head + num_patch + num_tail;

  // whole-segment assignment via Eigen block operations
  //   the degenerate branch above (fewer than 2 valid rows -> 2-point spline) still yields num_patch>=0 here
  Eigen::MatrixXd final_pos(T_new, 12);
  // final_pos[0 .. num_head-1] = pos_all[0 .. num_head-1]
  ...
  // final_pos[num_head+num_patch .. end] = pos_all[targets.back() .. end], num_tail rows in total
  ...
  //   normal case: all rows valid and exactly 4 segments
  targets_out.clear();
  if (static_cast<int>(valid_rows.size()) == P + 2 && n_steps_all.size() == 4) {
    ...
    //   num_head above is a 1-indexed count -> subtract 1 to convert to a 0-indexed offset
    ...
    // degenerate: approximate via round(linspace(num_head, num_head+num_patch+1, 5))
...
void AvoidanceSystem::run_optimization()
{
  ...
  // start/goal pre-check (0.4 threshold + warning + return)
...
void AvoidanceSystem::export_unified(const std::string& prefix, int level) const
{
  ...
  // ---- 1. meta.csv: parameter snapshot (key,value in columns; solver name is in the directory name) ----
  {
    std::vector<std::string> keys = {
      ...
      "lag_wd","lag_lam0","lag_s0","lag_tol_phys_margin","lag_tol_stable","lag_tol_stat","lag_max_iter",
    ...
    V << danger_threshold_, collision_tolerance_, fix_tolerance_,
         ...
         lag_wd_, lag_lam0_, lag_s0_, lag_tol_phys_, lag_tol_stable_, lag_tol_stat_,
  ...
  //   ⚠ the pure Lagrangian is a single main loop (no ALM inner/outer two levels), so here step = main-loop iteration
  {
    ...
    Eigen::MatrixXd I(std::max(total, 1), 22);
    int row = 0;
    for (int k = 0; k < n_outer; ++k) {
      const SolverLog& lg = iter_log_[k].solver_log;
      ...
      for (int s = 0; s < lg.total_iter; ++s) {
        ...
        I(row,21) = g(lg.cond_H_history, s);   // [Newton] cond(H) (non-zero only when verbose, otherwise NaN → recorded as 0)
        ++row;
      }
    }
    write_csv(dir + "/inner.csv",
      {"round","iter","L","f","f_a","f_b","penalty","maxD","G_norm","d_norm","alpha",
       ...
       "lam_max","lam_min","S_max","S_min","cond_H"}, I);
  }
