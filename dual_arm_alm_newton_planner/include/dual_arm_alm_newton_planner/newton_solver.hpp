...
#define DUAL_ARM_ALM_NEWTON_PLANNER_NEWTON_SOLVER_HPP
...
struct SolverLog {
  ...
  std::vector<double> alpha_history;     // step length (always 1 for Newton)
...
private:
  ...
  void compute_D_cache(const Eigen::VectorXd& X,
                       ...
                       std::vector<Eigen::MatrixXd>& D_minus) const;  // 3 return values (central-difference cache)
...
#endif  // DUAL_ARM_ALM_NEWTON_PLANNER_NEWTON_SOLVER_HPP
