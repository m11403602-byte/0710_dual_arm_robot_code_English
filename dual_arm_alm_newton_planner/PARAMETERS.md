# <p align="center">Parameter Reference — dual_arm_alm_newton_planner</p>

<p align="justify">
All tunable parameters of this planner reside in <code>config/dual_arm_alm_newton_planning.yaml</code> (this table = the yaml comments + supplementary notes). Modification workflow: edit the yaml → copy it to <code>hiwin_dual_arm/config/</code> → restart move_group for the changes to take effect ( <b> no recompilation needed </b> ).
</p>
...
> <ul>
>   <li align="justify" style="margin-bottom: 8px;"><b> Divergence </b> (values blow up) → first lower <code>alm_beta_c</code> , then lower <code>alm_c_max</code> , and finally adjust <code>alm_c0</code> .</li>
>   <li align="justify" style="margin-bottom: 8px;"><b> Fails to converge in time </b> (the outer rounds are exhausted before the target is met) → increase <code>alm_k_outer</code> .</li>
>   <li align="justify" style="margin-bottom: 8px;"><b> Persistently unable to converge </b> (stuck in place) → first raise <code>alm_beta_c</code> , then raise <code>alm_c_max</code> .</li>
> </ul>
>
...
> <ul>
>   <li align="justify" style="margin-bottom: 8px;">Divergence: a divergence warning appears, ‖G‖ explodes, and the trajectory jumps erratically.</li>
>   <li align="justify" style="margin-bottom: 8px;">Fails to converge in time: all <code>alm_k_outer</code> rounds are used up while maxD <b> is still decreasing </b> but has not yet dropped below <code>danger_threshold</code> .</li>
>   <li align="justify" style="margin-bottom: 8px;">Persistently unable to converge: maxD <b> stalls at some value </b> for several rounds, and c has long since reached <code>alm_c_max</code> .</li>
> </ul>
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="justify">Penalty-parameter upper bound (unified to 2000 across the three solvers). <b> Larger </b> → deep collisions can be pushed down further; <b> smaller </b> → safer, but may stall when overlaps are deep; ⚠ should not be excessively large: the larger c is, the more the penalty-term Hessian dominates → the condition number of H deteriorates → the Newton direction obtained from the LDLT solve becomes inaccurate</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td align="justify">Penalty amplification factor c ← beta_c·c (unified to 2 across the three solvers; must be > 1). <b> Larger </b> → the penalty climbs quickly and convergence is fast but prone to divergence; <b> smaller (near 1) </b> → climbs slowly, more stable but requiring more rounds; ⚠ should not be excessively large: amplifying c too quickly raises the condition number prematurely, so a gentle schedule is recommended for Newton</td>
    </tr>
    <tr>
...
<table width="100%">
  ...
  <tbody>
    <tr>
      ...
      <td align="justify">Export root directory: <code>&lt;prefix&gt;/&lt;unix-seconds&gt;_Newton/</code> (each export forms its own timestamped folder, so exports never overwrite one another; nothing is exported when export_level=0)</td>
    </tr>
    <tr>
