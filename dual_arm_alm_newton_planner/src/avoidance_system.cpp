...
// =====================================================================
// given endpoint slopes -> clamped cubic spline
...
// =====================================================================
// constructor: derive robot bases, store parameters, generate initial trajectory
// =====================================================================
    ...
    smooth_w_T_(smooth_w_T), smooth_w_neighbor_(smooth_w_neighbor)
{
  ...
  // base: Arm A Ty(700)Rz(180), Arm B Ty(-700)Rz(0)
                                      ...
                                      Eigen::MatrixXd* path_D_all) const
{
  ...
  // first compute step 1 to obtain the D length (full constraints)
  //   run once through NewtonSolver's FK+calc_df+mask to obtain num_D
...
CollisionIndices AvoidanceSystem::find_collision_targets(const Eigen::VectorXd& path_D) const
{
  ...
  Head = std::max(Head, 0);       // clamp to first valid 0-indexed position
  Tail = std::min(Tail, n - 1);   // clamp to last valid 0-indexed position
...
// =====================================================================
// run_solver_global: call the inner Newton
// =====================================================================
                                        ...
                                        SolverLog& solver_log)
{
  ...
  // P = number of interior points = size(X,1)-2; unpack V_final into per-point 12-dim rows (column-major)
                                                         ...
                                                         std::vector<int>& targets_out) const
{
  ...
  //   valid_rows = retained anchor indices
  ...
  // degenerate case: fewer than 2 valid anchors (anchors nearly all coincident)
  //   keep only head and tail; Clean_dist_seg = 1e-6; t_knots = [0; 1e-6]
  ...
  if (static_cast<int>(valid_rows.size()) < 2) {
    ...
    // normal case: Clean = retained anchor rows; t_knots = [0; cumsum(dist_seg)]
  ...
  // ===== Concatenation =====
  // patch_core = Patch_Pos with head and tail dropped
  //   num_patch = patch_core row count = Patch_Pos.rows() - 2  (key: use the actual row count, not T_patch)
  ...
  // num_tail = number of rows from targets.back() to the end (inclusive, 0-indexed)
  ...
  //   the degenerate branch above (fewer than 2 valid anchors -> 2-point spline) already ensures num_patch>=0
  Eigen::MatrixXd final_pos(T_new, 12);
  // final_pos's first num_head rows = pos_all[0 .. num_head-1]
  ...
  // final_pos's trailing rows = pos_all starting at targets.back(), num_tail rows in total
  ...
  //   normal case: all anchors valid and exactly 4 segments
  targets_out.clear();
  if (static_cast<int>(valid_rows.size()) == P + 2 && n_steps_all.size() == 4) {
    ...
    //   num_head above is a 1-indexed count -> subtract 1 to convert to a 0-indexed position
    ...
    // degenerate: evenly spaced indices from num_head to num_head+num_patch+1
...
// =====================================================================
// export_unified: consolidated export [NEW] (replaces the old four exporters; the sole public entry point)
...
void AvoidanceSystem::export_unified(const std::string& prefix, int level) const
{
  ...
  // ---- 4. danger_final.csv: initial vs. final MaxD comparison ----
  {
